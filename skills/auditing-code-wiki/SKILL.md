...
- **Page size:** any single wiki page larger than 200 lines is flagged as an oversize signal with a repair proposal in the correct authority direction — Reference pages may be compacted from verified source, while Spec compaction or a domain split requires user approval. Size alone never deletes an approved requirement. When the user asks to repair a flagged oversized page, do not compact it directly from the audit: follow the `updating-code-wiki` oversize flow — report first, then offer review together, split, delegate, or accept.
